...
HOURS   : 4 snapshots per forecast day (6-hourly), averaged to approximate
          a daily mean matching the ERA5 climatology baseline.
...
N_FCST_DAYS = 7
HOURS_PER_DAY = [6, 12, 18, 24]   # offsets within each 24-hr block
...
# All forecast hours needed (28 total)
ALL_FCST_HOURS = [h for d in range(1, N_FCST_DAYS + 1) for h in day_hours(d)]
...
def extract_ugrd_from_grib(grib_bytes: bytes) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    ...
    Returns (lats, lons, u_array shape (nlev, nlat, nlon), actual_levels_hpa).
    """
    import xarray as xr
...
def interp_to_climo_grid(lats_gefs: np.ndarray, aam: np.ndarray,
    ...
    """
    Interpolate a GEFS AAM profile (0.5° grid) to the climo grid (2.5°).
    Handles ascending/descending lat ordering robustly.
    """
...
def main() -> None:
    ...
    # all_aam[day][member] = list of aam arrays (one per snapshot hour), to be averaged
    # Structure: {day_idx: {member_idx: [aam_snapshot, ...]}}
    ...
    for day_idx in range(N_FCST_DAYS):
        ...
        for fxx in hours:
            ...
            for mem in range(N_MEMBERS):
                ...
                if lats_ref is None:
                    lats_ref = lats
                # interpolate to climo grid immediately to save memory
    ...
    for day_idx in range(N_FCST_DAYS):
        # Representative forecast hour for date calculation = f024 of this day
        fxx_mid   = day_idx * 24 + 24
        fcst_dt   = cycle_dt + timedelta(hours=fxx_mid)
        fcst_date = fcst_dt.date()
        doy       = doy365(fcst_date) - 1

        # Average snapshots per member to get daily mean AAM
        member_daily = []
        for mem in range(N_MEMBERS):
            snaps = day_member_snaps[day_idx][mem]
            if not snaps:
                continue
            member_daily.append(np.mean(snaps, axis=0))   # (nlat,)

        if not member_daily:
            log(f"  Day {day_idx + 1}: no members — skipping")
            continue

        stack    = np.array(member_daily)          # (n_mem, nlat)
    ...
    write_fcst_files(fcst_dates, lats_out,
